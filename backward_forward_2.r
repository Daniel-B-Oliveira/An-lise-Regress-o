...
# Quando não atender tentar:
# - tranformação das variáveis: passar para ln, termos quadráticos


modelo=lm(Y~X2+X3+X4,data=dados)
shapiro.test(residuals(modelo))     # H_0: normalidade    VS  H_1: rej normal
library(lmtest)
dwtest(modelo) # o pressuposto de independência é rejeitado   # H_0 independentes
...
library(lmtest)
dwtest(modelo) # o pressuposto de independ?ncia ? rejeitado   # a 5%

bptest(modelo) 

# A 1% todos os pressupostos ok para o modelo anteiror


# Ignorar a 1% a partir daqui
